...
impl std::fmt::Display for Protocol {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Protocol::Message(_, msg) => write!(f, "\n{:#?}", msg),
            Protocol::ChangeRoom(_, room) => write!(f, "\n{:#?}", room),
            Protocol::Fight(_, fight) => write!(f, "\n{:#?}", fight),
            Protocol::PVPFight(_, pvp_fight) => write!(f, "\n{:#?}", pvp_fight),
            Protocol::Loot(_, loot) => write!(f, "\n{:#?}", loot),
            Protocol::Start(_, start) => write!(f, "\n{:#?}", start),
            Protocol::Error(_, error) => write!(f, "\n{:#?}", error),
            Protocol::Accept(_, accept) => write!(f, "\n{:#?}", accept),
            Protocol::Room(_, room) => write!(f, "\n{:#?}", room),
            Protocol::Character(_, character) => write!(f, "\n{:#?}", character),
            Protocol::Game(_, game) => write!(f, "\n{:#?}", game),
            Protocol::Leave(_, leave) => write!(f, "\n{:#?}", leave),
            Protocol::Connection(_, connection) => write!(f, "\n{:#?}", connection),
            Protocol::Version(_, version) => write!(f, "\n{:#?}", version),
        }
    }
}
